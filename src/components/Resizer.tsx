...
function areasFromGridLines(gridLines: number[]) {
  return [...doubles(gridLines)].map(([start, end]) => end - start);
}
...
function gridResize(
  ...
  if (keepOuterCoords) {
    ...
    areas = areas.map((x, _, arr) => (x += remainingDim / arr.length));
  }
...
>) {
  ...
  }, [dragIdx, dir, minLength, onResize, unitLength, gridElement]);
  ...
  const cb: MouseEventHandler<HTMLDivElement> = useCallback(
    (e) => {
